...
export default function JudgeJamInterface() {
  ...
  // Start/stop timer and recording (with speech recognition)
  const toggleTimer = async () => {
    ...
    if (!isTimerRunning) {
      ...
      if (window && ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
        ...
        recog.onstart = () => setIsRecognizing(true)
      ...
      // Merge live transcript into transcript state
  ...
  // Poll for result if resultId is set
  useEffect(() => {
    if (!resultId) return
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`http://localhost:4000/api/result/${resultId}`)
        if (res.ok) {
  ...
  const handleSubmit = async () => {
    if (!isValidGithubUrl(githubUrl) || !selectedRubric) return
    if (!mediaBlob) {
      setSubmitResult('Please record your pitch before submitting.')
      return
    }
    setLoading(true)
    ...
    try {
      ...
      if (res.ok && data.resultId) {
        setSubmitResult("Submission successful! Waiting for analysis...")
          ...
          <PixelBorder variant="medium" color="dark">
            <div className="bg-light-blue">
              ...
              <div className="p-4 bg-light-blue">
                ...
                {isRecognizing && (
                  <div className="mb-2 p-2 bg-purple-100 border-l-4 border-purple-accent text-purple-900 font-mono text-sm animate-pulse">
                    <span className="font-bold">Live Transcript:</span> {liveTranscript}
                  </div>
                )}
                <Textarea
                  value={isRecognizing ? (transcript + ' ' + liveTranscript).trim() : transcript}
        ...
        <PixelBorder variant="large" color="dark">
          <div className="bg-light-blue">
            <div className="p-6 flex flex-col items-center gap-6 bg-medium-blue">
              ...
              {submitResult && (
                <p className="text-white text-sm mt-2">{submitResult}</p>
              )}
              {resultLoading && (
                <p className="text-white text-sm mt-2">Analyzing... Please wait.</p>
              )}
              {resultData && (
                <div className="text-white text-sm mt-2 bg-dark-blue p-4 rounded">
                  <h4 className="font-bold text-lg mb-2">AI Analysis Result</h4>
